...
@section('breadcrumb')
    ...
    <span class="text-secondary-900 font-medium">Editar Rol</span>
@endsection

@section('content')
    <div class="max-w-4xl mx-auto">
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-secondary-900">Editar Rol</h1>
            <p class="mt-1 text-sm text-secondary-500">Modifique la configuración del rol</p>
        </div>

        <form action="{{ route('accesscontrol.roles.update', $role) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <x-card title="Información del Rol" class="mb-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <p class="text-sm text-secondary-900 font-mono bg-secondary-50 rounded-lg px-3 py-2">{{ $role->code }}</p>
