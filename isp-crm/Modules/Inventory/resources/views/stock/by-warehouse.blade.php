...
@section('breadcrumb')
    ...
    <svg class="w-4 h-4 text-secondary-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" /></svg>
    <a href="{{ route('inventory.warehouses.show', $warehouse) }}" class="text-secondary-500 hover:text-secondary-700">{{ $warehouse->name }}</a>
    <svg class="w-4 h-4 text-secondary-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" /></svg>
...
@section('content')
    <div class="space-y-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-secondary-900">Stock - {{ $warehouse->name }}</h1>
                <p class="mt-1 text-sm text-secondary-500">Inventario del almacén</p>
            </div>
            <a href="{{ route('inventory.warehouses.show', $warehouse) }}"><x-button variant="secondary" icon="arrow-left">Volver al Almacén</x-button></a>
        </div>

        <x-card>
            <form method="GET" action="{{ route('inventory.stock.byWarehouse', $warehouse) }}" class="space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="flex items-end">
                        <label class="inline-flex items-center">
                            ...
                            <span class="ml-2 text-sm text-secondary-700">Solo con disponibilidad</span>
                        </label>
                    </div>
                </div>
                <div class="flex justify-end gap-2">
                    <a href="{{ route('inventory.stock.byWarehouse', $warehouse) }}"><x-button variant="ghost" type="button">Limpiar</x-button></a>
        ...
        <x-card :padding="false">
            <x-table>
                <x-slot name="header">
                    ...
                    <th class="px-6 py-3 text-left text-xs font-semibold text-secondary-700 uppercase">Categoría</th>
                ...
                @forelse($stock as $item)
                    <tr class="hover:bg-secondary-50">
                        ...
                        <td class="px-6 py-4 text-right text-sm"><span class="font-medium {{ $item->available_quantity > 0 ? 'text-success-600' : 'text-danger-600' }}">{{ number_format($item->available_quantity) }}</span></td>
                    </tr>
                @empty
                    <tr><td colspan="5" class="px-6 py-12 text-center"><div class="flex flex-col items-center"><x-icon name="package" class="w-12 h-12 text-secondary-300" /><h3 class="mt-2 text-sm font-medium text-secondary-900">Sin stock</h3><p class="mt-1 text-sm text-secondary-500">Este almacén no tiene productos en stock.</p></div></td></tr>
                @endforelse
            </x-table>
            @if($stock->hasPages())
